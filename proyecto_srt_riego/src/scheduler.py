...
class Scheduler:
    def __init__(self, sensor: SensorInterface, ctrl: Controller, act: Actuator, store: Storage, sup: Supervisor):
        ...
        self.act_sem = asyncio.Semaphore(1)
        self.db_sem = asyncio.Semaphore(1)
    ...
    async def _actuate(self, action: str):
        async with self.act_sem:
            if action == 'ON':
                await asyncio.to_thread(self.act.turn_on)
            elif action == 'OFF':
                await asyncio.to_thread(self.act.turn_off)

    async def _log_event(self, ev: Event):
        async with self.db_sem:
            await asyncio.to_thread(self.store.insert_event, ev)

    async def _flush_measurements(self, batch):
        async with self.db_sem:
            await asyncio.to_thread(self.store.insert_measurements, batch)
                ...
                if ok:
                    ...
                    if dec.action == 'ON':
                        with deadline_timer(Timing.ACTUATION_DEADLINE, 'ACTUATION'):
                            await self._actuate('ON')
                        await self._log_event(Event(ts=reading.ts, action='ON', reason=dec.reason))
                    elif dec.action == 'OFF':
                        with deadline_timer(Timing.ACTUATION_DEADLINE, 'ACTUATION'):
                            await self._actuate('OFF')
                        await self._log_event(Event(ts=reading.ts, action='OFF', reason=dec.reason))
                else:
        ...
        while not self._stop.is_set():
            ...
            with deadline_timer(Timing.FAULT_DEADLINE, 'FAULT-CHECK'):
                if self.sup.fault():
                    ...
                    if dec.action == 'OFF':
                        await self._actuate('OFF')
                        await self._log_event(Event(ts=time.time(), action='OFF', reason=dec.reason))
    ...
    async def _storage_task(self):
        ...
        while not self._stop.is_set():
            ...
            with deadline_timer(Timing.STORAGE_DEADLINE, 'STORAGE'):
                batch = list(self.buf)
                self.buf.clear()
                await self._flush_measurements(batch)
            dt = time.perf_counter() - t_start
            await asyncio.sleep(max(0.0, period - dt))
